      ...
      {balances.length === 0 ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
          {balances.map((b) => (
            ...
            >
              <TokenIcon token={b.token} size={32} />
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 600, fontSize: '14px' }}>{b.token.symbol}</div>
                <div style={{ fontSize: '12px', color: 'var(--text-muted)' }}>{b.token.name}</div>
              </div>
              <div style={{ textAlign: 'right' }}>
                <div style={{ fontWeight: 600, fontSize: '14px' }}>{b.balanceFormatted}</div>
              </div>
            </div>
          ))}
        </div>
